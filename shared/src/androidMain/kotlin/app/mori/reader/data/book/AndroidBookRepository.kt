...
@Composable
actual fun rememberBookRepository(): BookRepository {
    val applicationContext = LocalContext.current.applicationContext
    return remember(applicationContext) {
        AndroidBookRepository(applicationContext)
    }
}

@Composable
actual fun rememberEpubPicker(
...
private class AndroidBookRepository(
    ...
    override suspend fun loadCatalog(): BookCatalog = withContext(Dispatchers.IO) {
        booksRoot.mkdirs()
        buildCatalog()
    }

    override suspend fun loadReaderBook(bookId: String): ReaderBook = withContext(Dispatchers.IO) {
        booksRoot.mkdirs()
        val bookDir = findBookDirectory(bookId)
            ?: throw IllegalArgumentException("图书不存在")
        val metadata = loadBookMetadataStorage(bookDir)
            ?: throw IllegalArgumentException("图书不存在")
    ...
    override suspend fun importBooks(uriStrings: List<String>): BookCatalog = withContext(Dispatchers.IO) {
        ...
        uriStrings.forEach { uriString ->
            ...
            try {
                ...
                val finalCoverPath = metadata.coverPath
                    ?.let { coverPath ->
                        val coverFile = File(coverPath)
                        File(bookDir, coverFile.name).absolutePath
                    }
                    ?.takeIf { File(it).isFile }
                val bookMetadata = BookMetadataStorage(
                    id = bookId,
    ...
    override suspend fun saveReaderProgress(
    ...
    ): BookCatalog = withContext(Dispatchers.IO) {
        val bookDir = findBookDirectory(bookId) ?: return@withContext buildCatalog()
        val metadata = loadBookMetadataStorage(bookDir) ?: return@withContext buildCatalog()
        val readerBook = parseReaderBook(
            sourceFile = null,
            bookDir = bookDir,
            book = computeBookInfo(
                metadata,
        ...
        if (readerBook.chapters.isEmpty()) return@withContext buildCatalog()
    ...
    override suspend fun createCategory(name: String): BookCatalog = withContext(Dispatchers.IO) {
        val trimmed = name.trim()
        require(trimmed.isNotBlank()) { "分类名称不能为空" }
        val categories = loadCategories()
        if (categories.any { it.name == trimmed }) return@withContext buildCatalog()
        saveCategories(
    ...
    override suspend fun renameCategory(id: String, name: String): BookCatalog = withContext(Dispatchers.IO) {
        val trimmed = name.trim()
        require(trimmed.isNotBlank()) { "分类名称不能为空" }
    ...
    override suspend fun reorderCategories(categoryIds: List<String>): BookCatalog = withContext(Dispatchers.IO) {
        val categories = loadCategories()
    ...
    override suspend fun deleteCategory(id: String): BookCatalog = withContext(Dispatchers.IO) {
    ...
    override suspend fun updateBookCategories(bookId: String, categoryIds: List<String>): BookCatalog = withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteBook(bookId: String): BookCatalog = withContext(Dispatchers.IO) {
    ...
    private fun buildCatalog(): BookCatalog {
        ...
        val books = listBookDirectories().mapNotNull { bookDir ->
    ...
    private fun loadBookCategoryMap(): Map<String, List<String>> =
        runCatching {
            json.decodeFromString(BookCategoryMapStorage.serializer(), bookCategoryMapFile.readText()).books
                .mapValues { (_, categoryIds) -> categoryIds.distinct() }
        }.getOrDefault(emptyMap())

    private fun saveBookCategoryMap(bookCategoryMap: Map<String, List<String>>) {
        booksRoot.mkdirs()
        val normalized = bookCategoryMap
            .mapValues { (_, categoryIds) -> categoryIds.distinct() }
    ...
    private fun parseEpub(sourceFile: File, bookDir: File, fallbackTitle: String): EpubMetadata =
        ZipFile(sourceFile).use { zip ->
            val container = zip.readTextEntry("META-INF/container.xml")
                ?: return@use EpubMetadata(title = fallbackTitle)
            val rootPath = parseContainerRootPath(container)
                ?: return@use EpubMetadata(title = fallbackTitle)
            val opf = zip.readTextEntry(rootPath)
                ?: return@use EpubMetadata(title = fallbackTitle)
            val document = runCatching { parseXml(opf) }
                .getOrNull()
                ?: return@use EpubMetadata(title = fallbackTitle)
            val title = document.elementsByLocalName("title")
                .firstOrNull { it.textContent.isNotBlank() }
                ?.textContent
                ?.trim()
                ?: fallbackTitle
            val author = document.elementsByLocalName("creator")
                .firstOrNull { it.textContent.isNotBlank() }
                ?.textContent
                ?.trim()
            val coverPath = runCatching { extractCover(zip, document.documentElement, rootPath, bookDir) }
                .getOrNull()
            EpubMetadata(title = title, author = author, coverPath = coverPath)
        }

    private fun processBook(sourceFile: File, bookDir: File): ProcessedReaderBook =
        ZipFile(sourceFile).use { zip ->
            val container = zip.readTextEntry("META-INF/container.xml")
            ...
            val manifestItems = packageElement.elementsByLocalName("item")
                .map { item ->
                    ManifestItem(
                ...
                .associateBy { it.id }
            val spineElement = packageElement.elementsByLocalName("spine").firstOrNull()
                ?: throw IllegalStateException("EPUB 缺少 spine")
            val spineItems = spineElement.elementsByLocalName("itemref")
                .mapNotNull { itemref ->
            ...
            ProcessedReaderBook(
                ...
                info = ReaderBookInfoStorage(
                    ...
                    chapterInfo = chapters.associate { chapter ->
                        chapter.href to ReaderChapterInfoStorage(
    ...
    private fun parseReaderBook(sourceFile: File?, bookDir: File, book: BookInfo): ReaderBook {
        val storedInfo = loadReaderBookInfoStorage(bookDir)
        val storedToc = loadReaderTocStorage(bookDir)
        val processed = if (storedInfo == null) {
        ...
        val chapters = readerInfo.chapterInfo.entries
        ...
        val bookmark = loadBookmark(bookDir)
            ?.let {
                it.copy(
        ...
        return ReaderBook(
            info = book,
            chapters = chapters,
            tableOfContents = tableOfContents.ifEmpty {
                chapters.map {
                    ReaderTocItem(
                        label = it.title,
    ...
    private fun computeBookInfo(
        ...
        val currentChapterName = chapterIndex?.let { index ->
            readerInfo?.chapterInfo?.values?.firstOrNull { it.spineIndex == index }?.title
        }
        val progressPercent = if (readerInfo != null && bookmark != null && readerInfo.characterCount > 0) {
    ...
    private fun saveBookMetadataStorage(bookDir: File, metadata: BookMetadataStorage) {
    ...
    private fun saveReaderBookInfoStorage(bookDir: File, info: ReaderBookInfoStorage) {
    ...
    private fun saveReaderTocStorage(bookDir: File, toc: List<ReaderTocItem>) {
        File(bookDir, TOC_FILE)
            .writeText(json.encodeToString(ListSerializer(ReaderTocItem.serializer()), toc))
    }

    private fun ensureExtracted(zip: ZipFile, bookDir: File) {
    ...
    private fun saveBookmark(bookDir: File, bookmark: ReaderBookmark) {
    ...
    private fun extractCover(
        ...
        val coverId = packageElement.elementsByLocalName("meta")
            .firstOrNull { it.getAttribute("name") == "cover" }
            ?.getAttribute("content")
        val coverItem = manifestItems.firstOrNull { "cover-image" in it.properties.split(' ') }
        ...
        val extension = coverItem.href.substringAfterLast('.', "")
    ...
    private fun parseTableOfContents(
        ...
        val navRows = if (navItem != null) {
            val navPath = resolveZipPath(opfDir, navItem.href)
            zip.readTextEntry(navPath)
    ...
    private fun parseNavToc(
        ...
        val nav = document.elementsByLocalName("nav")
            .firstOrNull { nav ->
    ...
    private fun parseNcxPoints(
        ...
        parent.directChildElementsByLocalName("navPoint").forEach { point ->
            val label = point.elementsByLocalName("text").firstOrNull()?.textContent?.trim().orEmpty()
            val src = point.elementsByLocalName("content").firstOrNull()?.getAttribute("src").orEmpty()
    ...
    private fun appendTocRow(
        ...
        val chapterIndex = spineItems.indexOfFirst { item ->
    ...
    private fun resolveZipPath(baseDir: String, href: String): String {
        ...
        val parts = (if (baseDir.isBlank()) decoded else "$baseDir/$decoded")
            .split('/')
...
private fun String.filteredCharacterCount(): Int {
    var text = this
    Regex("<body\\b[^>]*>.*?</body>", RegexOption.DOT_MATCHES_ALL)
        .find(text)
        ?.let { text = it.value }
    text = text
        .replace(Regex("<rt\\b[^>]*>.*?</rt>", RegexOption.DOT_MATCHES_ALL), "")
        .replace(Regex("<(script|style)\\b[^>]*>.*?</\\1>", RegexOption.DOT_MATCHES_ALL), "")
        .replace(Regex("<[^>]+>"), "")
        .replace("&nbsp;", " ")
        .replace("&amp;", "&")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
    return text.codePoints().filter { it.isReaderCharacter() }.count().toInt()
}

private fun Int.isReaderCharacter(): Boolean =
    this in '0'.code..'9'.code ||
        this in 'A'.code..'Z'.code ||
        this in 'a'.code..'z'.code ||
        this in 0x25CB..0x25EF ||
        this in 0x3005..0x303B ||
        this in 0x3041..0x309E ||
        this in 0x30A1..0x30FA ||
        this == 0x30FC ||
        this in 0xFF10..0xFF19 ||
        this in 0xFF21..0xFF3A ||
        this in 0xFF41..0xFF5A ||
        this in 0xFF66..0xFF9D ||
        this in 0x2E80..0x2FDF ||
        this in 0x3400..0x4DBF ||
        this in 0x4E00..0x9FFF ||
        this in 0xF900..0xFAFF ||
        this in 0x20000..0x2EBEF
